...
# - SVG files: Used for inline React components (apply-branding.sh)
...
#   logo-source-square.svg  - Square logo for inline SVG replacement
...
#   logo-source-wide.svg    - Wide logo for inline SVG replacement
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN} Branding Asset Generator${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
...
# =============================================================================
# Generate V2 Logo Structure
# =============================================================================
# Stirling PDF V2 expects logos in specific format:
...
echo ""
echo -e "${CYAN}========================================${NC}"
echo -e "${CYAN} V2 Logo Structure Info${NC}"
echo -e "${CYAN}========================================${NC}"
echo ""

if [[ -n "$SVG_WIDE" ]] && [[ -n "$SVG_SQUARE" ]]; then
    echo -e "${GREEN}SVG source files found!${NC}"
    echo ""
    echo "During Docker build, apply-branding.sh will:"
    echo "  - Copy logo-source-wide.svg as text logo variants:"
    echo "    → StirlingPDFLogoBlackText.svg"
    echo "    → StirlingPDFLogoWhiteText.svg"
    ...
    echo ""
    echo "Stirling PDF V2 requires SVG logos. Please provide:"
    echo "  - $BRANDING_DIR/logo-source-wide.svg   (for text logos)"
    echo "  - $BRANDING_DIR/logo-source-square.svg (for icon logos)"
    echo ""
    echo "If you have EPS files, they were auto-converted to SVG above."
fi
...
echo ""
echo -e "${BLUE}V2 Branding Notes:${NC}"
echo "  - Logo customization uses SVG files during Docker build"
echo "  - App name/description must be set via Admin UI after first login"
echo "  - Custom CSS hides unwanted footer links (Discord, GitHub, Survey)"
echo ""
echo -e "${GREEN}========================================${NC}"
